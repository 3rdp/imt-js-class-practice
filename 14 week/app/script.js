...
//JSON.data
//JSON.parse({"foo":"te4st"});

// var vasya = new User("Вася"); 
// var petya = new User("Петя"); // создали пользователя
// vasya.sayHi(); // пользователь умеет говорить "Привет"
// vasya.setName("foo1234");
...
function CoffeeMachine(power) {
  ...
  _self.run = function() {
    setTimeout(onReady, getBoilTime.call(_self));
  };

  _self.countLi = function(){
    $("ul li").each(function(){
      console.log( $(this).text() + _self.waterAmount);
    })
  }
